...
# 1. 读取数据
data_path = 'expanded_data_with_labels.xlsx'  # 确保路径正确
...
features = df.iloc[:, 1:].values  # 获取特征数据，形状为 (400, 80)
labels = df.iloc[:, 0].values     # 获取标签数据，形状为 (400,)
...
print(f'\nPerceptron Model Test Accuracy: {test_accuracy:.2f}')
...
plt.title('Perceptron Model Confusion Matrix (Normalized)')
plt.show()
