...
class MNIST_MLP(object):
    ...
    def build_model(self, batch_size=100, input_size=784,
        ...
        # fc1 的输入形状
        input_shape_fc1 = pycnnl.IntVector(4)
        input_shape_fc1[0] = batch_size
        input_shape_fc1[1] = 1
        input_shape_fc1[2] = 1
        input_shape_fc1[3] = input_size

        # fc1 的权重形状：输入大小 -> hidden1
        weight_shape_fc1 = pycnnl.IntVector(4)
        weight_shape_fc1[0] = batch_size
        weight_shape_fc1[1] = 1
        weight_shape_fc1[2] = input_size
        weight_shape_fc1[3] = hidden1

        # fc1 的输出形状： (batch_size, hidden1, 1, 1) 或 (batch_size, 1, 1, hidden1)
        output_shape_fc1 = pycnnl.IntVector(4)
        output_shape_fc1[0] = batch_size
        output_shape_fc1[1] = 1
        output_shape_fc1[2] = 1
        output_shape_fc1[3] = hidden1

        self.net.createMlpLayer('fc1', input_shape_fc1, weight_shape_fc1, output_shape_fc1)
        self.net.createReLuLayer('relu1')

        # ----- 第二层：全连接层 fc2 (hidden1 -> hidden2) -----
        # fc2 的输入形状：来自 fc1 的输出
        input_shape_fc2 = pycnnl.IntVector(4)
        input_shape_fc2[0] = batch_size
        input_shape_fc2[1] = 1
        input_shape_fc2[2] = 1
        input_shape_fc2[3] = hidden1

        # fc2 的权重形状：hidden1 -> hidden2
        weight_shape_fc2 = pycnnl.IntVector(4)
        weight_shape_fc2[0] = batch_size
        weight_shape_fc2[1] = 1
        weight_shape_fc2[2] = hidden1
        weight_shape_fc2[3] = hidden2

        # fc2 的输出形状
        output_shape_fc2 = pycnnl.IntVector(4)
        output_shape_fc2[0] = batch_size
        output_shape_fc2[1] = 1
        output_shape_fc2[2] = 1
        output_shape_fc2[3] = hidden2

        self.net.createMlpLayer('fc2', input_shape_fc2, weight_shape_fc2, output_shape_fc2)
        self.net.createReLuLayer('relu2')

        # ----- 第三层：全连接层 fc3 (hidden2 -> out_classes) -----
        # fc3 的输入形状：来自 fc2 的输出
        input_shape_fc3 = pycnnl.IntVector(4)
        input_shape_fc3[0] = batch_size
        input_shape_fc3[1] = 1
        input_shape_fc3[2] = 1
        input_shape_fc3[3] = hidden2

        # fc3 的权重形状：hidden2 -> out_classes
        weight_shape_fc3 = pycnnl.IntVector(4)
        weight_shape_fc3[0] = batch_size
        weight_shape_fc3[1] = 1
        weight_shape_fc3[2] = hidden2
        weight_shape_fc3[3] = out_classes

        # fc3 的输出形状： (batch_size, out_classes, 1, 1)
        output_shape_fc3 = pycnnl.IntVector(4)
        output_shape_fc3[0] = batch_size
        output_shape_fc3[1] = 1
        output_shape_fc3[2] = 1
        output_shape_fc3[3] = out_classes

        self.net.createMlpLayer('fc3', input_shape_fc3, weight_shape_fc3, output_shape_fc3)
        self.net.createReLuLayer('relu3')

        # ----- 添加 Softmax 层 -----
        self.net.createSoftmaxLayer('softmax', output_shape_fc3, axis=1)
